...
unsigned int power(unsigned long long int a,unsigned long long int b,unsigned long long int p){
	if(b==1) return a;
	else return ((unsigned long long )pow(a,b) % p);
}
int main() {
	unsigned long long int p1,g,s1,s2,x1,x2,sym1,sym2;
